...
function HomeScreen({ navigation }) {
      ...
      getDailyResetIndicatorStorage(setDailyResetIndicator).then((date) => {
        ...
        if (dailyTimeInterval >= 24) {
          getDailyGoals(setDailyGoals).then((goals) => {
            getPastDailyGoals(setPastDailyGoals).then((pastGoals) => {
              handleUpdatePastDailyGoals(goals, pastGoals, date);
            });
          });
        }
      });
        ...
        if (weeklyTimeInterval > 24 * 7) {
          getWeeklyGoals(setWeeklyGoals).then((goals) => {
            getWeeklyNotificationDayStorage(setWeeklyNotificationDay).then(
              (day) => {
                getPastWeeklyGoals(setPastWeeklyGoals).then((pastGoals) => {
                  handleUpdatePastWeeklyGoals(goals, pastGoals, date, day);
                });
              }
            );
          });
  ...
  const handleUpdatePastDailyGoals = (
    goals,
    pastGoals,
    onFocusResetDailyIndicator
  ) => {
    let newIndicator = new Date();
    let id_date = new Date(onFocusResetDailyIndicator);
    id_date.setDate(onFocusResetDailyIndicator.getDate());
    const id = id_date.toDateString();

    newIndicator.setHours(
      onFocusResetDailyIndicator.getHours(),
      onFocusResetDailyIndicator.getMinutes()
    );
    newIndicator.setSeconds(0);
    setDailyResetIndicator(newIndicator);
    setDailyResetIndicatorStorage(newIndicator);
    ...
    const updatedPastDailyGoals = [id, ...pastGoals];
  ...
  const handleUpdatePastWeeklyGoals = (
    goals,
    pastGoals,
    onFocusResetWeeklyIndicator,
    day
  ) => {
    let newIndicator = new Date();

    const weekEndDate = new Date(onFocusResetWeeklyIndicator);
    weekEndDate.setDate(onFocusResetWeeklyIndicator.getDate() + 7);
    const id =
      onFocusResetWeeklyIndicator.toDateString() +
      " - " +
    ...
    newIndicator.setDate(
      newIndicator.getDate() - ((newIndicator.getDay() + addition) % 7)
    );

    newIndicator.setHours(
      onFocusResetWeeklyIndicator.getHours(),
      onFocusResetWeeklyIndicator.getMinutes()
    );
    newIndicator.setSeconds(0);
    setWeeklyResetIndicator(newIndicator);
    setWeeklyResetIndicatorStorage(newIndicator);
    ...
    const updatedPastWeeklyGoals = [id, ...pastGoals];
